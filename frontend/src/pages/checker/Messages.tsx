...
export default function CheckerMessages() {
  const queryClient = useQueryClient();
  const [recipientEmail, setRecipientEmail] = useState('');
  const [subject, setSubject] = useState('');
  const [body, setBody] = useState('');

  const { data: messages } = useQuery('checker-messages', () =>
    messagingAPI.list().then((res) => res.data)
  );

  const sendMutation = useMutation((data: any) => messagingAPI.create(data), {
    onSuccess: () => {
      toast.success('Message sent');
      queryClient.invalidateQueries('checker-messages');
      setRecipientEmail('');
      setSubject('');
      setBody('');
    },
